...
1. 把待统计的所有错误文件放在一个文件夹中。确认其文件格式是 .htm
...
**程序文件夹中的config.cfg为配置文件，内容为键－值对的形式，可以右键单击选择用记事本打开。其中保存有三个配置信息：mrp_controller、mrp_element、word_dir**

* mrp_controller 表示需要进行统计的mrp_controller项，如有修改，可以直接增加或删除，但必须要和已有元素保持一致的格式。即用单引号包围，并用逗号与其它元素分隔，同时要保存于同个中括号[]中
