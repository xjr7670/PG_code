...
class Work_dir(object):
    ...
    def __init__(self):
        if os.path.exists('config.inc'):
            f = open('config.inc')
            exist_dir = f.read().strip()
            if exist_dir == '' or not os.path.exists(exist_dir.replace('\\', '\\\\')):
                print 'No valid folder directory exists after last usage.'
                f.close()
                os.remove('config.inc')
        ...
        if os.path.exists('config.inc'):
            ...
            if work_dir == '':
                ...
                with open('config.inc', 'w') as f:
                    work_dir = self.confirm_dir(work_dir)
                    f.write(work_dir)
        else:
